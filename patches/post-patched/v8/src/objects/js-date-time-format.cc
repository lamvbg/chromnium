...
DirectHandle<Object> JSDateTimeFormat::TimeZoneId(Isolate* isolate,
  ...
  if (time_zone == u"GMT") {
    ...
    // Chronium: ICU tzdata still emits deprecated aliases as canonical
    // (Asia/Saigon, Asia/Calcutta, ...) even though modern IANA + real
    // Chrome on modern OSes report the current canonical (Asia/Ho_Chi_Minh,
    // Asia/Kolkata, ...). iphey/pixelscan/etc. flag the alias as a
    // fingerprint anomaly. Force the modern canonical form here.
    static const struct { const char16_t* from; const char16_t* to; } kAlias[] = {
        {u"Asia/Saigon",   u"Asia/Ho_Chi_Minh"},
        {u"Asia/Calcutta", u"Asia/Kolkata"},
        {u"Asia/Katmandu", u"Asia/Kathmandu"},
        {u"Asia/Rangoon",  u"Asia/Yangon"},
        {u"America/Buenos_Aires", u"America/Argentina/Buenos_Aires"},
        {u"Europe/Kiev",   u"Europe/Kyiv"},
    };
    for (const auto& a : kAlias) {
        if (canonical_time_zone == icu::UnicodeString(a.from)) {
            canonical_time_zone = icu::UnicodeString(a.to);
            break;
        }
    }
  }
